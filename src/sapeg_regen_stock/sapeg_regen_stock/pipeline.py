...
def run(
    ...
    for produit in produits_stock:
        try:
            ...
            logger.warning(
                f"AVERTISSEMENT [{produit.id_produit}] : échec dérivation — {e}"
            )
    ...
    _ecrire_stock_enrichi(
        produits_valides, produits_exclus_derivation, stock_enrichi_path
    )
...
def _ecrire_stock_enrichi(
    ...
    try:
        ...
        logger.info(
            f"stock_enrichi.csv écrit : {len(valides)} valides, {len(exclus_derivation)} exclus"
        )
    except Exception as e:
